...
use crate::chain::{
    Block, BlockHeader, BlockHeaderWithProof, CheckpointBeacon, Hash, InventoryItem,
...
fn create_inventory_announcement_message(inv_args: InventoryAnnouncementWitnetArgs) -> Message {
    // Get inventory items (flatbuffers' types)
    let ftb_inv_items = inv_args.inventory.inventory();
    let len = ftb_inv_items.len();

    // Create empty vector of inventory items
    let mut inv_items = Vec::new();

    // Create all inventory items (Witnet's types) and add them to a vector
    for i in 0..len {
        let inv_item = create_inventory_item(ftb_inv_items.get(i));
...
// Create a Witnet's InventoryItem from a flatbuffers' InventoryItem
fn create_inventory_item(inv_item: protocol::InventoryItem) -> InventoryItem {
    // Create inventory item hash
    let hash = create_hash(inv_item.hash());

    // Create inventory Item
    match inv_item.type_() {
        protocol::InventoryItemType::Error => InventoryItem::Error(hash),
        protocol::InventoryItemType::Tx => InventoryItem::Tx(hash),
        protocol::InventoryItemType::Block => InventoryItem::Block(hash),
        protocol::InventoryItemType::DataRequest => InventoryItem::DataRequest(hash),
        protocol::InventoryItemType::DataResult => InventoryItem::DataResult(hash),
    }
}
...
fn create_inventory_announcement_flatbuffer(
    ...
    let ftb_inv_items: Vec<flatbuffers::WIPOffset<protocol::InventoryItem>> = inv_args
        .inventory
        .iter()
        .map(|inv_item: &InventoryItem| {
            // Create flatbuffers' hash bytes
            let hash = match inv_item {
                InventoryItem::Error(hash)
                | InventoryItem::Tx(hash)
                | InventoryItem::Block(hash)
                | InventoryItem::DataRequest(hash)
                | InventoryItem::DataResult(hash) => hash,
            };
            ...
            let ftb_type = match inv_item {
                InventoryItem::Error(_) => protocol::InventoryItemType::Error,
                InventoryItem::Tx(_) => protocol::InventoryItemType::Tx,
                InventoryItem::Block(_) => protocol::InventoryItemType::Block,
                InventoryItem::DataRequest(_) => protocol::InventoryItemType::DataRequest,
                InventoryItem::DataResult(_) => protocol::InventoryItemType::DataResult,
            };

            // Create flatbuffers inv vector
            protocol::InventoryItem::create(
                builder,
                &protocol::InventoryItemArgs {
                    type_: ftb_type,
                    hash: Some(ftb_hash),
                },
            )
...
fn create_inventory_request_flatbuffer(
    ...
    // Create vector of flatbuffers' inv items
    let ftb_inv_items: Vec<flatbuffers::WIPOffset<protocol::InventoryItem>> = get_data_args
        .inventory
        .iter()
        .map(|inv_item: &InventoryItem| {
            // Create flatbuffers' hash bytes
